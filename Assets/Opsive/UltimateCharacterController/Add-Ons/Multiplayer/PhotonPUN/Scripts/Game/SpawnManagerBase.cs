...
namespace Opsive.UltimateCharacterController.AddOns.Multiplayer.PhotonPun.Game
{
    ...
    public abstract class SpawnManagerBase : MonoBehaviourPunCallbacks, IOnEventCallback
    {
        ...
        [Tooltip("Specifies the location that the character should spawn.")] [SerializeField]
        protected SpawnMode m_Mode;

        [Tooltip("The position the character should spawn if the SpawnMode is set to FixedLocation.")] [SerializeField]
        ...
        [SerializeField]
        protected Vector3 m_SpawnLocationOffset = new Vector3(2, 0, 0);

        [Tooltip(
            "The grouping index to use when spawning to a spawn point. A value of -1 will ignore the grouping value.")]
        [SerializeField]
        protected int m_SpawnPointGrouping = -1;

        [Tooltip("The amount of time it takes until an inactive player is removed from the room.")] [SerializeField]
        protected float m_InactiveTimeout = 60;
        ...
        {
            get { return m_SpawnLocationOffset; }
            set { m_SpawnLocationOffset = value; }
        }

        public int SpawnPointGrouping
        {
            get { return m_SpawnPointGrouping; }
            set { m_SpawnPointGrouping = value; }
        }
        ...
        public void SpawnPlayer(Player newPlayer)
        {
            ...
            if (determineSpawnLocation)
            {
                ...
                {
                    if (!SpawnPointManager.GetPlacement(null, m_SpawnPointGrouping, ref spawnPosition,
                        ref spawnRotation))
                    {
                        Debug.LogWarning(
                            $"Warning: The Spawn Point Manager is unable to determine a spawn location for grouping {m_SpawnPointGrouping}. " +
            ...
            if (photonView.ViewID > 0)
            {
                ...
                Debug.Log($"set ownership to {photonView.Owner.UserId}");
                ...
                PhotonNetwork.RaiseEvent(PhotonEventIDs.PlayerInstantiation, data, m_RaiseEventOptions,
                    m_ReliableSendOption);
                ...
                EventHandler.ExecuteEvent("OnPlayerEnteredRoom", newPlayer, player);
            }
            else
            {
        ...
        public override void OnPlayerLeftRoom(Player otherPlayer)
        {
            ...
            if (m_ActorNumberByPhotonViewIndex.TryGetValue(otherPlayer.ActorNumber, out int index))
            {
                ...
                if (otherPlayer != null && otherPlayer.IsInactive)
                {
                    ...
                    var removeEvent = Scheduler.Schedule<Player>(m_InactiveTimeout,
                        (Player player) => { m_InactivePlayers.Remove(player); }, otherPlayer);
                    m_InactivePlayers.Add(otherPlayer,
                        new InactivePlayer(index, photonView.transform.position, photonView.transform.rotation,
                            removeEvent));
                }
        ...
        public void OnEvent(EventData photonEvent)
        {
            if (photonEvent.Code == PhotonEventIDs.PlayerInstantiation)
            {
                ...
                for (int i = 0; i < data.Length / 4; ++i)
                {
                    ...
                    var character = Instantiate(GetCharacterPrefab(player), (Vector3) data[i * 4],
                        (Quaternion) data[i * 4 + 1]);
                    ...
                    if (photonView.IsMine)
                    {
                        ...
                        PhotonNetwork.RaiseEvent(PhotonEventIDs.RemotePlayerInstantiationComplete,
                            photonView.Owner.ActorNumber, m_RaiseEventOptions, m_ReliableSendOption);
                    }
                    else
                    {
